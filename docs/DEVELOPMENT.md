...
一个面向 M5GO v2.6 / M5Stack Core Basic、Gray 的 ESP32 WiFi 小屏幕固件：显示 Claude Code / Codex CLI 的实时工作状态和用量。
...
`firmware/` 是 ESP32 固件，联网后每 5 秒轮询这个服务，把状态画到 320x240 的
ILI9342C 横向彩屏上。桌宠动画（GIF）的上传和解码**全部在设备板子上完成**，换形象不再需要
电脑参与（详见第 4 节）。
...
firmware/     M5GO v2.6 / M5Stack Core ESP32 固件 (PlatformIO + Arduino)，含板上 GIF 解码
...
- **左键点击** → 弹出 M5GO 屏幕的**实时镜像**：Mac 端用与固件完全相同的数据
...
## 2. 烧录 M5GO v2.6 / M5Stack Core ESP32 固件

已确认硬件为 **M5Stack Core Basic / Gray（M5GO v2.6）**：ESP32、16MB Flash、内置
320x240 ILI9342C SPI 屏幕。它不适用于 Core2、M5Stick、ESP8266 SD2 小电视或其他接线不同
的开发板。屏幕与扬声器均使用 M5GO 板载连接，无需外接接线。

`firmware/platformio.ini` 固定了 M5GO 的 SPI 引脚、16MB 分区表和 LittleFS 标签；除非移植到
不同硬件，不要修改这些设置。详细的 Flash/LittleFS 分区和提示音素材说明见
[`firmware/README-M5STACK.md`](../firmware/README-M5STACK.md)。
...
| POST | `/api/brightness` | `value=0..100` 设置并持久化背光亮度 |
| POST | `/api/volume` | `value=0..100` 设置并持久化完成提示音音量 |
| POST | `/api/preview-sound` | 播放一次完成提示音试听 |
| POST | `/api/quota-display` | `mode=used\|reset` 切换额度文字显示方式 |
...
为避免上传和解码占用过多堆内存，链路仍以**逐行流式**方式工作；ESP32 仅缓存当前绘制帧，
