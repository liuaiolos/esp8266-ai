...
- **左键托盘图标** → M5GO/M5Stack 屏幕实时镜像（额度环 + 桌宠动画 + 网速图 + 音乐页，
  与设备渲染同一份数据），底部附 自动/Claude/Codex/网速/音乐 快速切换、屏幕亮度、
  提示音音量及「状态灯…」；后者可分别调整五种底座状态灯的颜色与亮度
...
| `MirrorForm.cs` | `MirrorPopover.swift` | 设备画面镜像弹窗（需与 macOS 的 320x240 布局保持一致） |
